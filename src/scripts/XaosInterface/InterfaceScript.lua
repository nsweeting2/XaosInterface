...
xi = xi or {
    ...
    neededMSDP = { --all of the msdp variables we will be requesting from the server
        ...
        [[ASCEND_EXP]], [[ASCEND_EXP_MAX]], [[SHIELDER]], [[SHIELDER_MAX]],
        [[PARTY_MEM]], [[IN_ROOM]],
        },
...
local function buildGaugeArea()
    ...
    xi.gaugeAdjCon =  xi.gaugeAdjCon or Adjustable.Container:new({
        ...
        x = "0%", y = "85%",
        width = "100%", height = "15%",
        adjLabelstyle = "background-color:rgba(0,0,0,100%); border: 5px groove grey;",
        buttonstyle = [[
    ...
    xi.gaugeAdjCon:attachToBorder("bottom")
...
function xi.gaugePosAdjustment()
    ...
    if tonumber(msdp.SHIELDER) == 0 and tonumber(msdp.SHIELDER_MAX) == 0 then
        xi.shieldbar:hide()
        xi.shieldbar:stop()
        xi.hpbar:move(0,0)
